...
def resample(audio: np.ndarray, orig_sr: int, target_sr: int) -> np.ndarray:
    """Pure-numpy linear resampler — replaces librosa.resample to avoid numba."""
    if orig_sr == target_sr:
        return audio
    target_length = int(round(audio.shape[-1] * target_sr / orig_sr))
    if audio.ndim == 1:
        ...
        x_new = np.linspace(0, 1, target_length)
        return np.array(
            [np.interp(x_new, x_old, row).astype(audio.dtype) for row in audio]
        )
